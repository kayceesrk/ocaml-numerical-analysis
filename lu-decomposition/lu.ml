...
*)
let lup a0 =
  let a = Array_.copy a0 in
  let m, n = Array_.matrix_size a in
  let r = min m n in
  let p = Array_.init m (fun i -> i) in (* permutation indices *)
  let lu = Array_.make_matrix m n 0.0 in
  ...
  for j = 0 to r - 1 do
    ...
    if j <> j' then Array_.(swap p j j' ; swap a j j' ; swap lu j j');
