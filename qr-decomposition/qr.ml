...
let gemm_householder k x h =
  let t = length h in
  let update_row xi =
    let xi' = sub xi k t in
    iteri (fun j hj -> xi.(j+k) <- dot xi' hj) h
  in
  iter update_row x
...
let qr a =
  ...
  let q = make n [||] in (* orthogonal matrix *)
  for k = 0 to min m n - 1 do
    (* Compute householder transformation matrix [h]. *)
    let x = sub w.(k) k (n - k) in
    let y = make (length x) 0. in
    y.(0) <- copysign (sqrt (dot x x)) (~-. (x.(0)));
    let h = householder x y in
    (* Update orthogonal matrix [q] *)
    if k = 0 then blit h 0 q 0 n else gemm_householder k q h;
    (* Update working memory [w] *)
    w.(k).(k) <- y.(0);
    gemm_householder k (sub w (k + 1) (m - k - 1)) h
  done;
