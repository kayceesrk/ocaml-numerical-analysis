...
let dot = Array_.fold_left2 (fun acc xi yi -> acc +. xi *. yi) 0.0
...
let gemm x y =
  let m, k = Array_.matrix_size x in
  let k', n = Array_.matrix_size y in
  assert(k = k');
  Array_.map (gemv_t y) x

(** Transpose a given matrix. *)
let trans a =
  let m, n = Array_.matrix_size a in
  Array_.init_matrix n m (fun i j -> a.(j).(i))
...
(** [householder x y] returns householder transformation matrix [h] (such that
    [h * x] = [y] and [h * y] = [x]). *)
let householder x y =
  let z = Array_.map2 ( -. ) x y in
  let c = 2. /. dot z z in
  let h i zi j zj = (if i = j then 1. else 0.) -. c *. zi *. zj in
  Array_.mapi (fun i zi -> Array_.mapi (h i zi) z) z

(** [gemm_householder k x h] multiplies matrix [x] and householder matrix [h]
    (of the [k]-th iteration). *)
let gemm_householder k x h =
  let t = Array_.length h in
  let update_row xi =
    let xi' = Array_.sub xi k t in
    Array_.iteri (fun j hj -> xi.(j+k) <- dot xi' hj) h
  in
  Array_.iter update_row x
...
let qr a =
  ...
  let (m, n) = Array_.matrix_size w in
  let q = Array_.make n [||] in (* orthogonal matrix *)
  for k = 0 to min m n - 1 do
    (* Compute householder transformation matrix [h]. *)
    let x = Array_.sub w.(k) k (n - k) in
    let y = Array_.make (Array_.length x) 0. in
    y.(0) <- copysign (sqrt (dot x x)) (~-. (x.(0)));
    let h = householder x y in
    (* Update orthogonal matrix [q] *)
    if k = 0 then Array_.blit h 0 q 0 n else gemm_householder k q h;
    (* Update working memory [w] *)
    w.(k).(k) <- y.(0);
    gemm_householder k (Array_.sub w (k + 1) (m - k - 1)) h
  done;
  let r = Array_.init_matrix n m (fun i j -> if i <= j then w.(j).(i) else 0.) in
  (q, r)
